...
/**
 * Helper class to create common types of Haikunators.
 */
final class Haikunators
{
     ...
     *
     * @copyright Atrox, BSD-3-Clause License
     */
    public const ATROX_ADJECTIVES = [
    ...
    /**
     * List of nouns used by {@link https://github.com/Atrox/haikunatorphp Atrox/haikunatorphp}.
     *
     * @copyright Atrox, BSD-3-Clause License
     */
    public const ATROX_NOUNS = [
    ...
    /**
     * List of adjectives used by {@link https://github.com/jitsi/js-utils Jitsi}.
     *
     * @copyright Jitsi, Apache License 2.0
     */
    public const JITSI_ADJECTIVES = [
        ...
        'Digital', 'Diplomatic', 'Direct', 'Dirty', 'Disappointed',
        ...
        'General', 'Generic', 'Generous', 'Gentle', 'Genuine',
        ...
        'ThoughtProvoking', 'Thoughtful', 'Thrilled', 'Tidy',
        ...
        'Weird', 'Welcome', 'Well', 'West', 'Western', 'Wet', 'Whole',
    ...
    /**
     * List of nouns used by {@link https://github.com/jitsi/js-utils Jitsi}.
     *
     * @copyright Jitsi, Apache License 2.0
     */
    public const JITSI_NOUNS = [
        ...
        'Agents', 'Ages', 'Agreements', 'Agricultures', 'Aides',
        ...
        'Assemblies', 'Assertions', 'Assessments',
        ...
        'Averages', 'Awards', 'Awarenesses', 'Backdrops',
        'Backgrounds', 'Backings', 'Backs', 'Backups', 'Bacterias', 'Badges',
        'Bags', 'Bails', 'Balances', 'Ballets', 'Balloons', 'Ballots',
        ...
        'Biologies', 'Birds', 'Birthdays', 'Biscuits', 'Bishops',
        'Bites', 'Bits', 'Blades', 'Blames', 'Blankets', 'Blanks',
        'Blasts', 'Blends', 'Blessings', 'Blocks', 'Blogs', 'Blows',
        ...
        'Breakthroughs', 'Breathings', 'Breaths',
        ...
        'Bullets', 'Bunches', 'Burdens', 'Bureaucracies',
        ...
        'Donations', 'Donors', 'Doors', 'Doses', 'Dots', 'Doubts',
        ...
        'Nuts', 'Objections', 'Objectives', 'Objects',
        ...
        'Princes', 'Princesses', 'Principals', 'Principles',
        ...
        'Publics', 'Publishings', 'Pubs', 'Pulls', 'Pulses', 'Pumps',
        ...
        'Realities', 'Realizations', 'Realms', 'Reasonings', 'Reasons',
        ...
        'Representations', 'Representatives', 'Republics',
        ...
        'Rights', 'Rings', 'Riots', 'Rises', 'Risks', 'Rituals',
        ...
        'Rubbishes', 'Rugbies', 'Ruins', 'Rules', 'Rulings',
        ...
        'Shoppings', 'Shops', 'Shores', 'Shortages', 'Shots',
        ...
        'Strings', 'Strokes', 'Structures', 'Struggles', 'Students',
        ...
        'Virtues', 'Viruses', 'Visas', 'Visions', 'Visitors',
    ...
    /**
     * List of verbs used by {@link https://github.com/jitsi/js-utils Jitsi}.
     *
     * @copyright Jitsi, Apache License 2.0
     */
    public const JITSI_VERBS = [
        ...
        'Breathe', 'Bring', 'Broadcast', 'Brush', 'Build', 'Burn',
        'Burst', 'Buy', 'Calculate', 'Call', 'Calm', 'Camp', 'Campaign',
        ...
        'Shelter', 'Shift', 'Shine', 'Ship', 'Shock', 'Shop', 'Shout',
    ...
    /**
     * List of adverbs used by {@link https://github.com/jitsi/js-utils Jitsi}.
     *
     * @copyright Jitsi, Apache License 2.0
     */
    public const JITSI_ADVERBS = [
    ...
    /**
     * Creates a {@link Haikunator} that mostly behaves like the
     * {@link https://github.com/Atrox/haikunatorphp Atrox/haikunatorphp} library.
     * It can be used for easier library migration, but also if this style of generation is wanted.
     *
     * @param array $params parameters for generation configuration
     * @param string[] $adjectives list of adjectives to use for generation
     * @param string[] $nouns list of nouns to use for generation
     * @return Haikunator haikunator that produces output compatible with Atrox/haikunatorphp
     */
    public static function atroxHaikunator(
        array             $params = [],
        array             $adjectives = self::ATROX_ADJECTIVES,
        array             $nouns = self::ATROX_NOUNS,
        ?IntegerGenerator $integerGenerator = null,
    ): Haikunator
    {
        $integerGenerator ??= new MtRandIntegerGenerator();

        $haikunators = [];

        if (!empty($adjectives)) {
            $haikunators[] = new WordListHaikunator($integerGenerator, $adjectives);
        }
        if (!empty($nouns)) {
            $haikunators[] = new WordListHaikunator($integerGenerator, $nouns);
        }

        $tokenLength = $params['tokenLength'] ?? 4;
        if ($tokenLength > 0) {
            if ($params["tokenHex"] ?? false) {
                $tokenChars = "0123456789abcdef";
            } else {
                $tokenChars = $params['tokenChars'] ?? "0123456789";
            }
            $haikunators[] = new CharactersHaikunator($integerGenerator, $tokenChars, $tokenLength);
        }

        $delimiter = $params['delimiter'] ?? '-';
        return new AppendingHaikunator($haikunators, $delimiter);
    }

    /**
     * Creates a {@link Haikunator} that generates names like {@link https://jitsi.org/ Jitsi} room names.
     *
     * @param string[] $adjectives list of adjectives to use for generation
     * @param string[] $nouns list of nouns to use for generation
     * @param string[] $verbs list of verbs to use for generation
     * @param string[] $adverbs list of adverbs to use for generation
     * @return Haikunator haikunator that produces output looking like Jitsi room names
     */
    public static function jitsiHaikunator(
        array             $adjectives = self::JITSI_ADJECTIVES,
        array             $nouns = self::JITSI_NOUNS,
        array             $verbs = self::JITSI_VERBS,
        array             $adverbs = self::JITSI_ADVERBS,
        ?IntegerGenerator $integerGenerator = null,
    )
    {
        $integerGenerator ??= new MtRandIntegerGenerator();

        return new AppendingHaikunator([
            new WordListHaikunator($integerGenerator, $adjectives),
            new WordListHaikunator($integerGenerator, $nouns),
            new WordListHaikunator($integerGenerator, $verbs),
            new WordListHaikunator($integerGenerator, $adverbs),
        ]);
    }
}
